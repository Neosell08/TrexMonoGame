    ...
    public partial class GameInstance
    {
        protected override void Draw(GameTime gameTime)
        {
            ...
            if (CurState == GameState.MainMenu)
            {
                ...
                RenderTarget = ApplyEffect(CRTShader, RenderTarget, new SetShaderParamDelegate((shader) => { shader.Parameters["Time"].SetValue((float)Globals.Time.TotalGameTime.TotalSeconds); shader.Parameters["ChromaticStrength"].SetValue(ChromaticStrength); }));

                GraphicsDevice.SetRenderTarget(null);

                _spriteBatch.Begin();
                _spriteBatch.Draw(RenderTarget, new Rectangle(0, 0, (int)Globals.WindowResolution.X, (int)Globals.WindowResolution.Y), Color.White);
                _spriteBatch.End();
            ...
            else
            {
                ...
                foreach (LightningBossAttackObject lightning in LightningBossAttackObject.LightningObjects)
                {
                ...
                foreach (Projectile bullet in Projectile.Projectiles)
                {
                ...
                if (CurState == GameState.Dead || CurState == GameState.Won)
                {
                    _spriteBatch.Draw(Collider.PixelTexture, new Rectangle(0, 0, (int)Globals.WindowResolution.X, (int)Globals.WindowResolution.Y), new Color(2, 2, 2, 150));
                }
                ...
                RenderTarget2 = ApplyEffect(CRTShader, RenderTarget2, new SetShaderParamDelegate((shader) => { shader.Parameters["Time"].SetValue((float)Globals.Time.TotalGameTime.TotalSeconds); shader.Parameters["ChromaticStrength"].SetValue(ChromaticStrength); }));
                GraphicsDevice.SetRenderTarget(null);
                GraphicsDevice.Clear(Color.Transparent);
        ...
        {
            if (rt == null) { return; }
            _spriteBatch.Draw(rt, new Rectangle(0, 0, (int)Globals.WindowResolution.X, (int)Globals.WindowResolution.Y), Color.White);
        } 
    }
}
