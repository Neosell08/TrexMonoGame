...
public partial class GameInstance : Game
{
    ...
    protected override void Initialize()
    {
        ...
        Globals.ScreenResolution = new Vector2(GraphicsDevice.DisplayMode.Width, GraphicsDevice.DisplayMode.Height);
        Globals.WindowResolution = new Vector2(_graphics.PreferredBackBufferWidth, _graphics.PreferredBackBufferHeight);
    ...
    protected override void LoadContent()
    {
        ...
        ShootSeperator = new BasicGameObject(Globals.WindowResolution/2, 0, new Texture(Collider.PixelTexture, new Vector2(Globals.WindowResolution.X, 10)));
        
        CRTShader = Content.Load<Effect>("Resources/CRTShader");
        CRTShader.Parameters["PixelSize"].SetValue(8);
        CRTShader.Parameters["ScreenSize"].SetValue(Globals.ScreenResolution);
        Background = Content.Load<Texture2D>("Resources/Background");

        InitBoss(true);

        RenderTarget = new RenderTarget2D(GraphicsDevice, (int)Globals.WindowResolution.X, (int)Globals.WindowResolution.Y);
        RenderTarget2 = new RenderTarget2D(GraphicsDevice, (int)Globals.WindowResolution.X, (int)Globals.WindowResolution.Y);
        ...
        TimeText = new TextGameObject(smallerFont, "Time: 00 : 00", new Vector2(Globals.WindowResolution.X / 2, Globals.WindowResolution.Y / 2 ), textureScale);
        MainText = new TextGameObject(titleFont, "Dead", new Vector2(Globals.WindowResolution.X/2, Globals.WindowResolution.Y / 2 - (smallFont.MeasureString("Time: 00 : 00").Y/2 + titleFont.MeasureString("DEAD").Y/2)), textureScale);
        RecordText = new TextGameObject(smallerFont, "Personal Record: 00 : 00", new Vector2(Globals.WindowResolution.X / 2, Globals.WindowResolution.Y / 2 + smallFont.MeasureString("Time: 00 : 00").Y), textureScale);
        ...
        MoreSoonText = new TextGameObject(smallFont, "More Coming Soon!", new Vector2((Globals.WindowResolution.X - (Level1Icon.TopLeftCorner.X + Level1Icon.Textr.ScaledTextureScale.X) + 350) / 2, Globals.WindowResolution.Y / 2), textureScale);
        CountdownText = new TextGameObject(titleFont, "3", Globals.WindowResolution /2, textureScale);
    ...
    public void SetState(GameState state)
    {
        ...
        {
            List<Projectile> temp = Projectile.Projectiles.ToList();

            InitBoss();
            
            LightningBossAttackObject[] lightnings = LightningBossAttackObject.LightningObjects.ToArray();
    ...
    public void InitBoss(bool loadContent = false)
    {
        ...
        CurBoss = new Boss(new Vector2(250, 100), loadContent ? frames : CurBoss.Anim.Frames, new Texture(null, new Vector2(2, 2)), 23, 1, new Vector2[4] { new Vector2(100, 100), new Vector2(100, 200), new Vector2(Globals.WindowResolution.X - 100, 100), new Vector2(Globals.WindowResolution.X-100, 200) }, 250, 0.0075f, 5, 55, loadContent ? Content.Load<SoundEffect>("Resources/explosion") : CurBoss.DeathSound);
